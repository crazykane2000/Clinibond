<?php session_start();
    ...
    $lata = get_data_id_data("members", "id", $_REQUEST['id']);
?>
<!DOCTYPE html>
<html>
<head>
      ...
      <?php include 'navbar.php'; ?>
           ...
           <div class="col-sm-3 lft">
            <div style="padding: 20px;" class="mobss"></div>
              <div class="lft_pad">
                ...
                <h1 style="font-family: 'Century Gothic';color: #999;font-size: 25px;font-weight: normal;"><div style="font-weight: bold;color: #fff">Indian School of Democracy  </div><span style="font-size: 14px;">View Alloted Leads to <?php echo $lata['name']; ?> For Stage 2</span></h1>
              </div>
           </div>
           <div class="col-sm-9">
       ...
       <?php 
        ...
        <div style="padding: 20px;"></div>        
          ...
          <div class="col-md-12">
            <a href="view_alloted_leads.php?id=<?php echo $_REQUEST['id']; ?>" class="btn btn-success">Stage 1 </a>
            <a href="view_alloted_leads_stage_2.php?id=<?php echo $_REQUEST['id']; ?>" class="btn btn-success">Stage 2 </a>
            <div style="padding: 10px;"></div>
            <div class="card">
              <h3 style="font-weight: normal;font-size: 20px;color:#666;">
                Assigned Leads Stage 2 <span style="font-weight: bold;color: #333;"> (<?php echo $lata['name']; ?>)</span></h3><hr/>
              ...
              <div class="table-responsive">
               ...
               <table class="table table-hover table-striped" id="example" >
                  ...
                  <tbody>
                    ...
                    $drt = 'SELECT users.id, users.stage_1_clear,  users.strong_candidate, users.answer_status, users.marks, users.profile_status, users.document_status, users.date, users.stage_2_clear, users.name, users.email, users.phone, users.language, users.register_date, user_info.gender, user_info.age, user_info.present_state FROM users INNER JOIN user_info ON users.id=user_info.user_id WHERE users.profile_status="Done" AND  users.answer_status="Done" AND  users.document_status="Done" AND users.stage_2_alloted="Yes" AND users.alloted_to_stage_2="'.$lata['id'].'" ORDER BY users.name ASC';
                      ...
                      foreach ($user as $key => $value) {
                          ...
                          if($value['stage_2_clear']!="No" ){
                             $statys_stage_2 = '<a  style="background-color:red"  title="View all Information" class="badge">'.$value['stage_2_clear'].'</a>';                                                    
                          }
                        ...
                        echo ' <tr>
                                ...
                                <td style="text-transform:uppercase">'.$language.'</td>
                                 ...
                                 <td>'.$strong.'</td>
                                <td>                                  
                                  <a style="zoom:0.7" href="remove_lead_stage_2.php?id='.$value['id'].'&user_id='.$lata['id'].'" ><button class="btn btn-danger btn-sm" title="Remove Lead"><i class="icon-remove-sign"></i> Remove</button></a>
                                </td>
                              </tr>';
                              $i++;
                      }
