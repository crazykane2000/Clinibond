<?php session_start();
    ...
    $state = $_REQUEST['id'];
    $data = get_data_col("users", "language", $state);
    $states = array();
          ...
          <div class="col-md-12">
            <div class="card">
              <h3 style="font-weight: normal;font-size: 20px;color:#666;">
                Report Language Wise (<?php echo $_REQUEST['id']; ?>) </h3>
                 ...
                 <table class="table table-hover table-striped" id="example" >
                  ...
                  <tbody>
                    <?php 
                      ...
                      try {
                        if ($state=="") {
                           $stmt = $pdo->prepare( 'SELECT users.id, users.answer_status, users.profile_status, users.document_status, users.date, users.name, users.email, users.phone, users.language, users.register_date, user_info.gender, user_info.age, user_info.present_state FROM users INNER JOIN user_info ON users.id=user_info.user_id WHERE users.profile_status="Done" AND  users.answer_status="Done" AND  users.document_status="Done" AND users.language IS NULL ORDER BY users.name ASC');
                        }else{
                          $stmt = $pdo->prepare( 'SELECT users.id, users.answer_status, users.profile_status, users.document_status, users.date, users.name, users.email, users.phone, users.language, users.register_date, user_info.gender, user_info.age, user_info.present_state FROM users INNER JOIN user_info ON users.id=user_info.user_id WHERE users.profile_status="Done" AND  users.answer_status="Done" AND  users.document_status="Done" AND users.language = "'.$state.'" ORDER BY users.name ASC');

                          //$stmt = $pdo->prepare('SELECT * FROM `users` WHERE `language`="'.$state.'" ORDER BY date DESC');
                        }
