                 ...
                 <table class="table table-hover table-striped" id="example" >
                  ...
                  <tbody>
                    <?php 
                      ...
                      try {
                         $stmt = $pdo->prepare( 'SELECT user_info.present_state, COUNT(*) as county FROM users INNER JOIN user_info ON users.id=user_info.user_id WHERE users.profile_status="Done" AND  users.answer_status="Done" AND  users.document_status="Done" GROUP BY user_info.present_state');

                          //$stmt = $pdo->prepare('SELECT  present_state,  COUNT(*) as county FROM user_info GROUP BY present_state ORDER BY present_state');
                      ...
                      foreach ($user as $key => $value) {
                        echo ' <tr>
                                <td>'.$i.'</td>
                                <td><a style="color:#095a9a;text-decoration:underline" href="view_data_state.php?id='.$value['present_state'].'"  title="View all Information"><b>'.$value['present_state'].'</b></a></td> 
                                <td><a style="color:#095a9a;text-decoration:underline" href="view_data_state.php?id='.$value['present_state'].'"  title="View all Information"><b>'.$value['county'].'</b></a></td>                                
                              </tr>';
                              $i++;
                      }
                     ?>
