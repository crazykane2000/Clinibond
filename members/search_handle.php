                 ...
                 <table class="table table-hover table-striped" id="example" >
                  ...
                  <tbody>
                    ...
                    $user_age = $_REQUEST['age'];
                     ...
                     //echo 'SELECT users.id, users.answer_status, users.profile_status, users.document_status, users.date, users.name, users.email, users.phone, users.language, users.register_date, user_info.gender, user_info.age, user_info.present_state FROM users INNER JOIN user_info ON users.id=user_info.user_id WHERE user_info.gender LIKE "'.$_REQUEST['gender'].'%" AND users.language LIKE "%'.$_REQUEST['language'].'%" AND user_info.present_state LIKE "%'.$_REQUEST['state'].'%" '. $age;
                      try {
                          if($_REQUEST['complete_status']=="Completed"){
                               $stmt = $pdo->prepare('SELECT users.id, users.answer_status, users.profile_status, users.document_status, users.date, users.name, users.email, users.phone, users.language,user_info.present_city, user_info.present_org, user_info.religion,  user_info.sc_st, user_info.finance_support, user_info.sc_st_desc,  user_info.origin, user_info.present_business, users.register_date, user_info.gender, user_info.age, user_info.present_state FROM users INNER JOIN user_info ON users.id=user_info.user_id WHERE user_info.gender LIKE "'.$_REQUEST['gender'].'%" AND users.language LIKE "%'.$_REQUEST['language'].'%" AND user_info.present_state LIKE "%'.$_REQUEST['state'].'%" '. $age.' AND users.answer_status = "Done" AND users.profile_status = "Done" AND users.document_status="Done"'); 
                          }else{
                               $stmt = $pdo->prepare('SELECT users.id, users.answer_status, users.profile_status, users.document_status, users.date, users.name, users.email, users.phone, users.language,user_info.present_city, user_info.present_org, user_info.religion,  user_info.sc_st, user_info.finance_support, user_info.sc_st_desc, user_info.origin, user_info.present_business, users.register_date, user_info.gender, user_info.age, user_info.present_state FROM users INNER JOIN user_info ON users.id=user_info.user_id WHERE user_info.gender LIKE "'.$_REQUEST['gender'].'%" AND users.language LIKE "%'.$_REQUEST['language'].'%" AND user_info.present_state LIKE "%'.$_REQUEST['state'].'%" '. $age); 
                          }
                         
                          //.echo 'SELECT users.id, users.answer_status, users.profile_status, users.document_status, users.date, users.name, users.email, users.phone, users.language, users.register_date, user_info.gender, user_info.age, user_info.present_state FROM users INNER JOIN user_info ON users.id=user_info.user_id WHERE user_info.gender LIKE "'.$_REQUEST['gender'].'%" AND user_info.present_state LIKE "%'.$_REQUEST['state'].'%" AND '. $user_age;
                      ...
                      foreach ($user as $key => $value) {
                         ...
                         echo ' <tr>
                                ...
                                <td><b>'.$value['origin'].'</b> </td>                                 
                                 ...
                                 <td>'.date($value['date'],strtotime('-330 minutes', 0)).'</td>
                                <td>
