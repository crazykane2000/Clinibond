                 ...
                 <table class="table table-hover table-striped" id="example" >
                  ...
                  <tbody>
                    <?php 
                      ...
                      try {
                          $stmt = $pdo->prepare( 'SELECT user_info.gender, COUNT(*) as county FROM users INNER JOIN user_info ON users.id=user_info.user_id WHERE users.profile_status="Done" AND  users.answer_status="Done" AND  users.document_status="Done" GROUP BY user_info.gender');
                      ...
                      foreach ($user as $key => $value) {
                        echo ' <tr>
                                <td>'.$i.'</td>
                                <td><a style="color:#095a9a;text-decoration:underline" href="view_data_gender.php?id='.$value['gender'].'"  title="View all Information"><b>'.$value['gender'].'</b></a></td> 
                                <td><a style="color:#095a9a;text-decoration:underline" href="view_data_gender.php?id='.$value['gender'].'"  title="View all Information"><b>'.$value['county'].'</b></a></td>                                
                              </tr>';
                              $i++;
                      }
                     ?>
